...
public class Metrics {
	...
	static {
		...
		if(System.getProperty("bstats.relocatecheck") == null || !System.getProperty("bstats.relocatecheck").equals("false")) {
			// Maven's Relocate is clever and changes strings, too. So we have to use
			// this
			// little "trick" ... :D
			final String defaultPackage = new String(new byte[]{'o', 'r', 'g', '.', 'b', 's', 't', 'a', 't', 's', '.', 'b', 'u', 'k', 'k', 'i', 't'});
			final String examplePackage = new String(new byte[]{'y', 'o', 'u', 'r', '.', 'p', 'a', 'c', 'k', 'a', 'g', 'e'});
			// We want to make sure nobody just copy & pastes the example and use the
			// wrong
			// package names
	...
	public Metrics(Plugin plugin, int pluginId) {
		...
		if(enabled) {
			boolean found = false;
			// Search for all other bStats Metrics classes to see if we are the first
			// one
	...
	private void startSubmitting() {
		...
		timer.scheduleAtFixedRate(new TimerTask() {
			...
			public void run() {
				...
				// Don't be afraid! The connection to the bStats server is still async,
				// only the
	...
	public JsonObject getPluginData() {
		...
		data.addProperty("pluginVersion", pluginVersion); // Append the version of
		// the plugin
	...
	private static void sendData(Plugin plugin, JsonObject data) throws Exception {
		...
		connection.addRequestProperty("Content-Encoding", "gzip"); // We gzip our
		// request
